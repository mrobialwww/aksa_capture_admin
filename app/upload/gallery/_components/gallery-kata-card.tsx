...
interface GalleryKataCardProps {
  index: number
  word: string
  gerakanTab: 'benar' | 'salah'
}

export function GalleryKataCard({ index, word, gerakanTab }: GalleryKataCardProps) {
  const isBenar = gerakanTab === 'benar'

  return (
    <Link
      href={`/upload/gallery/kata/${encodeURIComponent(word)}?is_correct=${isBenar}`}
    ...
    >
      <Card className="flex min-h-[160px] flex-col justify-between gap-0 rounded-[20px] p-0 shadow-sm ring-1 ring-border/60 transition-shadow hover:shadow-md">
        ...
        <div className="flex flex-col items-center pb-2 pt-1">
          <p className="text-[22px] font-bold text-foreground">{word}</p>
        </div>
