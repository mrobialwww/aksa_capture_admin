...
interface GalleryHurufCardProps {
  letter: string
  gerakanTab: 'benar' | 'salah'
}

export function GalleryHurufCard({ letter, gerakanTab }: GalleryHurufCardProps) {
  const isBenar = gerakanTab === 'benar'

  return (
    <Link
      href={`/upload/gallery/huruf/${encodeURIComponent(letter)}?is_correct=${isBenar}`}
